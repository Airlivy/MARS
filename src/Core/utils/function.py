...
def create_logger(file_path=None):
    ...
    if file_path is None:
        # 兜底路径：锚定本文件所在目录，而非当前工作目录
        fallback_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'process', 'output')
        os.makedirs(fallback_dir, exist_ok=True)
        file_path = os.path.join(fallback_dir, 'output.log')
